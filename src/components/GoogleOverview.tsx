...
// ── KPI summary cards ─────────────────────────────────────────────────────────
function GoogleKPICards({ creatives }: { creatives: AdCreative[] }) {
  ...
  const cards = [
    { label: "Total Spend",   value: formatCurrency(spend,     "DKK", true), color: "text-[#4285F4]" },
    { label: "Conversions",   value: formatNumber(leads),                     color: "text-success" },
    { label: "CPA",           value: formatCurrency(cpa,       "DKK"),        color: "text-text-primary" },
    { label: "Impressions",   value: formatNumber(impressions, true),          color: "text-text-primary" },
    { label: "CTR",           value: formatPercent(ctr, 2),                   color: ctr >= 3 ? "text-success" : "text-warning" },
...
function SpendConversionsChart({ daily }: { daily: MetaDailyPoint[] }) {
  ...
  return (
    <div className="card">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-semibold text-sm text-text-primary">Spend & Conversions Over Time</h3>
          <p className="text-xs text-text-muted">Last 90 days</p>
        </div>
        <div className="flex gap-4">
          <div className="flex items-center gap-1.5 text-xs text-text-muted">
            <div className="w-3 h-0.5 rounded" style={{ background: GBLUE }} /> Spend
          </div>
          <div className="flex items-center gap-1.5 text-xs text-text-muted">
            <div className="w-3 h-0.5 rounded" style={{ background: GREEN }} /> Conversions
          </div>
        </div>
      </div>
        ...
        <AreaChart data={data} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id="gGSpend" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%"  stopColor={GBLUE} stopOpacity={0.25} />
              <stop offset="95%" stopColor={GBLUE} stopOpacity={0} />
            </linearGradient>
          ...
          <YAxis yAxisId="conversions" orientation="right" tick={{ fontSize: 10, fill: "#6b7280" }} tickLine={false} axisLine={false} />
          <Tooltip
          ...
          />
          <Area yAxisId="spend"       type="monotone" dataKey="spend"       stroke={GBLUE} strokeWidth={2} fill="url(#gGSpend)" dot={false} />
          <Area yAxisId="conversions" type="monotone" dataKey="conversions" stroke={GREEN} strokeWidth={2} fill="url(#gGConv)"  dot={false} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
function CampaignChart({ creatives }: { creatives: AdCreative[] }) {
  const map = new Map<string, { name: string; spend: number; conversions: number }>();
  for (const c of creatives) {
    const e = map.get(c.campaignId);
    if (e) { e.spend += c.spend; e.conversions += c.leads; }
    else map.set(c.campaignId, { name: c.campaignName, spend: c.spend, conversions: c.leads });
  }
    ...
    <div className="card">
      ...
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={data} layout="vertical" margin={{ top: 0, right: 32, left: 0, bottom: 0 }}>
          ...
          />
          <Bar dataKey="spend"       fill={GBLUE} radius={[0, 3, 3, 0]} maxBarSize={16} />
          <Bar dataKey="conversions" fill={GREEN} radius={[0, 3, 3, 0]} maxBarSize={16} />
        </BarChart>
      </ResponsiveContainer>
      <div className="flex gap-4 mt-1 justify-end">
        <div className="flex items-center gap-1.5 text-xs text-text-muted">
          <div className="w-3 h-2 rounded-sm" style={{ background: GBLUE }} /> Spend
        </div>
        <div className="flex items-center gap-1.5 text-xs text-text-muted">
          <div className="w-3 h-2 rounded-sm" style={{ background: GREEN }} /> Conversions
        </div>
      </div>
    </div>
  );
...
function GooglePipeline({ leads }: { leads: Lead[] }) {
  const googleLeads = leads.filter(l => l.source === "google");
  const total       = googleLeads.length;
  const evaluation  = googleLeads.filter(l => l.currentBoard === "evaluation").length;
  const sales       = googleLeads.filter(l => l.currentBoard === "sales").length;
  const booked      = googleLeads.filter(l => l.currentBoard === "aftercare").length;
  const stuck       = googleLeads.filter(l => l.isStuck).length;

  return (
    <div className="card flex flex-col gap-4">
      <div>
        <h3 className="font-semibold text-sm text-text-primary">Google → Monday Pipeline</h3>
        <p className="text-xs text-text-muted mt-0.5">
          {total} leads from Google tracked in Monday CRM
        </p>
      </div>

      {total === 0 ? (
        <p className="text-xs text-text-muted text-center py-6">
          No Google leads found in Monday yet — make sure the "Source" column is set to Google for these leads.
        </p>
      ) : (
        <>
          <div className="space-y-3">
            <PipelineBar label="In Evaluation"   count={evaluation} total={total} color={GBLUE} />
...
export function GoogleOverview({
  ...
  return (
    <div className="space-y-6">
      <GoogleKPICards creatives={creatives} />
      <SpendConversionsChart daily={daily} />
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <CampaignChart creatives={creatives} />
        <GooglePipeline leads={leads} />
      </div>
    </div>
  );
}
